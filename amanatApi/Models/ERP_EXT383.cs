using System;
using System.Collections.Generic;

#nullable disable

namespace amanatApi.Models
{
    public partial class ERP_EXT383
    {
        public int ID { get; set; }
        public int? ASSETID { get; set; }
        public string MAHRAMANENO { get; set; }
        public int FUELTYPE { get; set; }
        public string BENZINCARD { get; set; }
        public string CNGCARD { get; set; }
        public string GASOLINECARD { get; set; }
        public int? CYLINDER { get; set; }
        public int? WHEEL { get; set; }
        public int? AXIS { get; set; }
        public int? CAPACITY { get; set; }
        public int? COLORID { get; set; }
        public int? KARBARIID { get; set; }
        public string VIN { get; set; }
        public string AGHLAM { get; set; }
        public string PROPERTIES { get; set; }
        public string CARD { get; set; }
        public int? SANADVAZIATID { get; set; }
        public int? SANADTYPEID { get; set; }
        public int? MALEKIATTYPEID { get; set; }
        public string MALEK { get; set; }
        public int? SANADTEDAD { get; set; }
        public string VMALEKIAT { get; set; }
        public string SANADREMARK { get; set; }
        public string POLLUTION { get; set; }
        public int? MOTORVOLUME { get; set; }
        public int? CYLINDERARRANGEMENT { get; set; }
        public string MOTORMODEL { get; set; }
        public int? HP { get; set; }
        public int? GASHTAVAR { get; set; }
        public int? GEARBOX { get; set; }
        public string BREAKSYSTEM { get; set; }
        public int? BREAKFRONT { get; set; }
        public int? BREAKREAR { get; set; }
        public string TIREFRONT { get; set; }
        public string TIREREAR { get; set; }
        public string RING { get; set; }
        public int? CARCLASS { get; set; }
        public int? BODYCLASS { get; set; }
        public int? SUPAP { get; set; }
        public int? BAKVOLUME { get; set; }
        public int? GEARS { get; set; }
        public string SUSPENSION { get; set; }
        public int? SUSPENSIONFRONT { get; set; }
        public int? SUSPENSIONREAR { get; set; }
        public string FANARFRONT { get; set; }
        public string FANARREAR { get; set; }
        public string FARMAN { get; set; }
        public string IMENI { get; set; }
        public int? ENTEGHALGHODRAT { get; set; }
        public int? SOOKHTRESANI { get; set; }
        public int? AIRBAG { get; set; }
        public string DAFTARCODE { get; set; }
        public string DAFTERSABTNO { get; set; }
        public int? N_DAFTARSAFHE { get; set; }
        public int? DAFTARDATE { get; set; }
        public string FUELCARD { get; set; }
        public int? POLUTION { get; set; }
        public int? MALEKPID { get; set; }
        public string CAPACITYSTR { get; set; }
        public string PRODATE { get; set; }
    }
}
